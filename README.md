...
В результате исследования был готовый набор данных. Главные представители двух групп алгоритмов обнаружения объектов YOLOv5 и RESNET были обучены с использованием подготовленного набора данных. Экспериментальным образом были получены метрики моделей. Результаты показали, что модель YOLOv5 сохраняет высокую точность (P=0.917, R=0.754, mAP50=0.847) при различных входных данных, и имеет лучшую скорость обработки (FPS=0.424). Её должно быть достаточно для решения задачи обнаружения номера автомобиля, погрешность подсчета с такой точностью считается допустимой. 
